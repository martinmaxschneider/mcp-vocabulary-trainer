...
export default function NounsPage() {
  ...
  const existingNounCount =
    existingEntries?.entries.filter((e) => e.category === "NOUN").length ?? 0;
  ...
  const generateMutation = api.assist.generateCategorySuggestions.useMutation({
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: tToasts("nounsFound", { count: data.suggestions.length }),
      });
    },
    onError: (error) => {
      setIsGenerating(false);
  ...
  const handleAddSelected = async () => {
    ...
    if (selected.length === 0) {
      toast({
        ...
        description: tToasts("selectAtLeastOneNoun"),
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="max-w-5xl">
      <div className="mb-8">
        ...
        <h1 className="mb-2 text-4xl font-bold">{t("nounsTitle")}</h1>
        <p className="text-muted-foreground">{t("nounsSubtitle", { language: sourceLanguageName })}</p>
        {existingNounCount > 0 && (
          <div className="mt-4">
            <Badge variant="secondary" className="text-sm">
              <BookOpen className="mr-1 h-3 w-3" />
              {t("nounsExisting", { count: existingNounCount })}
            </Badge>
          </div>
        )}
      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {t("nounsGenerateTitle")}
              </CardTitle>
              <CardDescription>{t("nounsGenerateDesc", { language: sourceLanguageName })}</CardDescription>
            </CardHeader>
                ...
                <CardContent className="pt-6">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      <span className="font-medium">{t("nounsAdding")}</span>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>{t("nounsSelectDesc")}</CardDescription>
                  </div>
